...
quiz_data = [
    # 수식: \frac{A}{B}
    ("유리식 $\\frac{x^2 - 1}{x + 1}$은 분자와 분모가 모두 다항식이므로 유리식이다.", True, "유리식은 (다항식) / (다항식) 꼴로 분자와 분모가 모두 다항식이므로 유리식입니다."),
    ("다항식 $3x + 5$는 유리식이 아니다.", False, "다항식은 분모가 $1$인 유리식($\\frac{3x+5}{1} = 3x+5$)에 포함되므로 유리식입니다."),
    ("$\frac{5}{\sqrt{x}}$는 유리식이다.", False, "분모 $\sqrt{x}$는 다항식이 아니므로 유리식이 아닙니다. 문자의 지수가 음이 아닌 정수여야 다항식입니다."),
    ("유리식 $\\frac{2x}{x-3}$는 $x=3$에서 성립한다.", False, "유리식은 분모가 $0$이 되는 값($x=3$)에서는 정의되지 않아 성립하지 않습니다. 이를 정의역에서 제외합니다."),
    ("유리식 $\\frac{x-1}{x^2+1}$는 모든 실수 $x$에서 성립한다.", True, "분모 $x^2+1$은 항상 $1$ 이상이므로 $0$이 될 수 없습니다. 따라서 모든 실수에서 성립합니다."),
    ("$\\frac{x}{0}$는 유리식이다.", False, "분모에 상수 다항식 '0'은 올 수 없습니다. 수학적으로 분모가 0인 식은 정의되지 않습니다."),
...
def display_quiz():
    ...
    * **다항식도 유리식이다**: 분모 $B$가 $0$이 아닌 상수 다항식일 경우, 유리식은 다항식이 됩니다.
      **예시**: $\\frac{3x+5}{1} = 3x+5$
    * **분수식**: 유리식 중에서 다항식이 아닌 식, 즉 **분모에 문자가 포함**된 식을 특별히 분수식이라고 부릅니다.
    ...
    if st.session_state.current_question >= 0:
        ...
        question_text_with_latex = quiz_data[q_idx][0]
        
        st.subheader(f"문제 {len(st.session_state.quiz_history) + 1}")
        
        # Markdown을 사용하여 문제와 수식을 함께 표시합니다.
        st.markdown(question_text_with_latex) 

...
# 복사 가능한 원본 텍스트 내용 (일반 텍스트 분수 표기 사용)
...
1.  다항식 3x^2 - 2x + 5는 유리식이다. ( )
2.  유리식 (x+1)/(x-2)에서 x=2일 때의 식의 값은 3이다. ( )
3.  x/y + y/x는 유리식이다. (단, x는 0이 아니고, y는 0이 아니다) ( )
"""
...
with st.form("fill_in_the_blank_form"):
    st.markdown("""
    1.  **분수식:**
        두 다항식 $A$와 $B$ ($B \neq 0$)에 대하여, `A/B` 꼴로 나타낼 수 있는 식 중 **다항식이 아닌 식**을 **( A )** 이라고 합니다. (예: `1/x, (x-1)/(x^2+1)`)
    """)
...
if submitted_ox:
    ...
    st.write(f"**2. x=2일 때 (x+1)/(x-2)의 값은 3이다.** {results[2][1]}")
    st.caption("❌ 해설: x=2를 대입하면 분모가 2-2=0이 되므로, 식의 값이 **정의되지 않습니다.**")
    
    st.write(f"**3. x/y + y/x는 유리식이다.** {results[3][1]}")

...
# ----------------------------------------------------
# 0. 데이터 로드 및 이미지 경로 설정 함수
# ----------------------------------------------------

# (이전과 동일한 get_image_path, display_feedback_image 함수 유지)
...
# 1. 문제 데이터 (총 30개)
# ----------------------------------------------------
# 문제 데이터를 별도의 함수로 분리하여 로딩
def get_full_quiz_data():
    return [
        # OX 문제 15개 (데이터 간결화를 위해 일부만 표시)
...
def main():
    ...
    FULL_QUIZ_DATA = get_full_quiz_data() # 데이터 로드
    ...
    # '다시 풀기' 버튼을 누르면 세션 상태를 확실히 초기화하고 재실행
    if st.button("🔄 새로운 10문제로 다시 풀기", key='restart_btn_top'):
        restart_quiz()
        st.rerun() # 재실행을 통해 모든 상태를 리셋
    ...
    * **성립 조건**: 분모 $B$는 **절대 $0$이 아니어야** 합니다 ($B \\ne 0$).
    ...
    q_data = FULL_QUIZ_DATA[q_index] # 수정된 문제 데이터 로드 방식 적용

import streamlit as st
import numpy as np
import matplotlib.pyplot as plt

# 페이지 설정
st.set_page_config(
    page_title="이차함수 그래프 기본형 분석",
    layout="centered"
)

## 📌 이차함수 그래프 기본형 (y=ax^2) 분석하기

st.title('이차함수 그래프 기본형 ($y=ax^2$) 분석하기')

# ---

### 계수 'a' 값 선택 위젯

st.header('1. 계수 $a$ 값 선택')

# a 값 조절을 위한 슬라이더 생성. 0은 제외하도록 min_value, max_value, step 설정
a = st.slider(
    '계수 $a$ 값 (0 제외)',
    min_value=-5.0,  # 최소값
    max_value=5.0,   # 최대값
    value=1.0,       # 기본값
    step=0.1,        # 단계
    format='%.1f'    # 소수점 한 자리 표시
)

# a가 0인 경우를 막기 위한 처리
if a == 0:
    st.error("계수 $a$는 0이 될 수 없습니다. 0이 아닌 다른 값을 선택해 주세요.")
    st.stop() # 0일 경우 앱 실행 중지

st.markdown(f"**선택된 이차함수:** $\mathbf{{y = {a:.1f}x^2}}$")

# ---

### 그래프 그리기

st.header('2. 그래프 시각화')

# x 범위 설정 (예: -5부터 5까지)
x = np.linspace(-5, 5, 400)
# y = ax^2 계산
y = a * (x**2)

# Matplotlib으로 그래프 생성
fig, ax = plt.subplots(figsize=(8, 6))

# 그래프 그리기
ax.plot(x, y, label=f'$y = {a:.1f}x^2$', color='blue')

# 축 설정
ax.set_title(f'이차함수 그래프: $y = {a:.1f}x^2$')
ax.set_xlabel('$x$')
ax.set_ylabel('$y$')

# x축과 y축이 0을 포함하도록 설정
ax.axhline(0, color='gray', linewidth=0.8)
ax.axvline(0, color='gray', linewidth=0.8)

# 축 범위 설정 (y축은 a값의 변화를 잘 보여주기 위해 -25부터 25까지 설정)
ax.set_xlim([-5, 5])
ax.set_ylim([-25, 25])
ax.grid(True, linestyle='--', alpha=0.6)
ax.legend()
ax.set_aspect('equal', adjustable='box') # 그래프의 비율을 맞춤

# Streamlit에 그래프 표시
st.pyplot(fig)

# ---

### 그래프 분석 및 추론

st.header('3. $a$ 값에 따른 그래프 특징 분석')

st.markdown("슬라이더를 움직여 $a$ 값을 바꿔가며 그래프의 모양을 관찰하고 아래 내용을 확인하세요.")

if a > 0:
    # a가 양수일 때
    st.success(f"**$a = {a:.1f}$ (양수):** 그래프는 **아래로 볼록**하며, 꼭짓점은 원점 (0, 0)입니다.")
    st.markdown("이는 $x$의 모든 값에 대해 $y$의 값이 0 또는 양수($ax^2 \ge 0$)이기 때문입니다.")
else:
    # a가 음수일 때
    st.error(f"**$a = {a:.1f}$ (음수):** 그래프는 **위로 볼록**하며, 꼭짓점은 원점 (0, 0)입니다.")
    st.markdown("이는 $x$의 모든 값에 대해 $y$의 값이 0 또는 음수($ax^2 \le 0$)이기 때문입니다.")

st.markdown("---")

# 폭 분석
abs_a = abs(a)
st.subheader('$|a|$ 값과 폭')

st.markdown(f"현재 $|a| = |{a:.1f}| = **{abs_a:.1f}**$ 입니다.")

if abs_a > 1.5:
    st.info("$\mathbf{|a|}$의 값이 **크므로** ($|a| > 1.5$ 일 때), 그래프의 폭은 **좁아집니다**.")
elif abs_a < 0.5:
    st.warning("$\mathbf{|a|}$의 값이 **작으므로** ($|a| < 0.5$ 일 때), 그래프의 폭은 **넓어집니다**.")
else:
    st.info("$\mathbf{|a|}$의 값이 중간 정도이므로, 폭은 적당합니다.")

st.markdown(
    """
    * **귀납적 추론 포인트:**
        1.  $a$의 부호($a>0$ 또는 $a<0$)에 따라 **볼록한 방향**이 어떻게 달라지는지 확인해보세요.
        2.  $|a|$의 값이 클수록 (예: $a=4, -4$) 그래프가 **y축에 가까워져 폭이 좁아지는** 것을 확인해보세요.
        3.  $|a|$의 값이 작을수록 (예: $a=0.5, -0.5$) 그래프가 **x축에 가까워져 폭이 넓어지는** 것을 확인해보세요.
    """
)
